// This file is auto-generated, don't edit it. Thanks.
package com.aliyun.live20161101.models;

import com.aliyun.tea.*;

public class DescribeBoardSnapshotResponse extends TeaModel {
    @NameInMap("headers")
    @Validation(required = true)
    public java.util.Map<String, String> headers;

    @NameInMap("body")
    @Validation(required = true)
    public DescribeBoardSnapshotResponseBody body;

    public static DescribeBoardSnapshotResponse build(java.util.Map<String, ?> map) throws Exception {
        DescribeBoardSnapshotResponse self = new DescribeBoardSnapshotResponse();
        return TeaModel.build(map, self);
    }

    public DescribeBoardSnapshotResponse setHeaders(java.util.Map<String, String> headers) {
        this.headers = headers;
        return this;
    }
    public java.util.Map<String, String> getHeaders() {
        return this.headers;
    }

    public DescribeBoardSnapshotResponse setBody(DescribeBoardSnapshotResponseBody body) {
        this.body = body;
        return this;
    }
    public DescribeBoardSnapshotResponseBody getBody() {
        return this.body;
    }

}
