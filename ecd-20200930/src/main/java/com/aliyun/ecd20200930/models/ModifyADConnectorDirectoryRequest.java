...
public class ModifyADConnectorDirectoryRequest extends TeaModel {
    ...
    /**
     * <p>Details of the DNS addresses that correspond to the enterprise AD system. You can specify only one IP address. Make sure that the specified IP address can be accessed over the network that the selected vSwitch uses.</p>
     */
    @NameInMap("DnsAddress")
    public java.util.List<String> dnsAddress;

    /**
     * <p>The domain name. The domain name must be 2 to 255 characters in length and consist of two parts. Example: example.com. You can register each domain name only once.</p>
     */
    ...
    /**
     * <p>Specifies whether to enable multi-factor authentication (MFA). Default value: false.</p>
     * <br>
     * <p>If you enable MFA, all AD users in the AD directory must enter the password and the dynamic verification code generated by the MFA device when they connect to a cloud desktop.</p>
     * <br>
     * <p>> The first time you log on to the Alibaba Cloud Workspace client as an end user, you must bind an MFA device.</p>
     */
    ...
    /**
     * <p>The DNS addresses of the enterprise AD subdomain. If you specify SubDomainName but not this parameter, the DNS address of the subdomain is considered to be the same as that of the parent domain.</p>
     */
