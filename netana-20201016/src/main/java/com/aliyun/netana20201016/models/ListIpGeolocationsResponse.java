// This file is auto-generated, don't edit it. Thanks.
package com.aliyun.netana20201016.models;

import com.aliyun.tea.*;

public class ListIpGeolocationsResponse extends TeaModel {
    @NameInMap("headers")
    @Validation(required = true)
    public java.util.Map<String, String> headers;

    @NameInMap("body")
    @Validation(required = true)
    public ListIpGeolocationsResponseBody body;

    public static ListIpGeolocationsResponse build(java.util.Map<String, ?> map) throws Exception {
        ListIpGeolocationsResponse self = new ListIpGeolocationsResponse();
        return TeaModel.build(map, self);
    }

    public ListIpGeolocationsResponse setHeaders(java.util.Map<String, String> headers) {
        this.headers = headers;
        return this;
    }
    public java.util.Map<String, String> getHeaders() {
        return this.headers;
    }

    public ListIpGeolocationsResponse setBody(ListIpGeolocationsResponseBody body) {
        this.body = body;
        return this;
    }
    public ListIpGeolocationsResponseBody getBody() {
        return this.body;
    }

}
