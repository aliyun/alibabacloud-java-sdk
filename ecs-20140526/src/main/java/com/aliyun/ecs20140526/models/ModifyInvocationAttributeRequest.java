...
public class ModifyInvocationAttributeRequest extends TeaModel {
     ...
     * </ul>
     * </li>
     * <li><p>You can specify built-in environment parameters as custom parameters. When the command is executed, Cloud Assistant automatically replaces them with the corresponding values in the environment without manual assignment. The following built-in environment parameters are supported:</p>
     * <ul>
     * <li><code>{{ACS::RegionId}}</code>: The region ID.</li>
     * <li><code>{{ACS::AccountId}}</code>: The UID of the Alibaba Cloud account.</li>
     * <li><code>{{ACS::InstanceId}}</code>: The instance ID. When the command is sent to multiple instances, to use <code>{{ACS::InstanceId}}</code> as a built-in environment parameter, make sure that the Cloud Assistant Agent version is not earlier than the following versions:<ul>
     * <li>Linux: 2.2.3.309</li>
     * <li>Windows: 2.1.3.309</li>
     * </ul>
     * </li>
     * <li><code>{{ACS::InstanceName}}</code>: The instance name. When the command is sent to multiple instances, to use <code>{{ACS::InstanceName}}</code> as a built-in environment parameter, make sure that the Cloud Assistant Agent version is not earlier than the following versions:<ul>
     * <li>Linux: 2.2.3.344</li>
     * <li>Windows: 2.1.3.344</li>
     * </ul>
     * </li>
     * <li><code>{{ACS::InvokeId}}</code>: The command execution ID. To use <code>{{ACS::InvokeId}}</code> as a built-in environment parameter, make sure that the Cloud Assistant Agent version is not earlier than the following versions:<ul>
     * <li>Linux: 2.2.3.309</li>
     * <li>Windows: 2.1.3.309</li>
     * </ul>
     * </li>
     * <li><code>{{ACS::CommandId}}</code>: The command ID. When you call this operation to execute a command, to use <code>{{ACS::CommandId}}</code> as a built-in environment parameter, make sure that the Cloud Assistant Agent version is not earlier than the following versions: <ul>
     * <li>Linux: 2.2.3.309</li>
     * <li>Windows: 2.1.3.309</li>
     * </ul>
     * </li>
    ...
    /**
     * <p>The encoding type of the command content (CommandContent). Valid values (case-insensitive):</p>
     * <ul>
     ...
     * <ul>
     * <li>When you enable custom parameters or modify the custom parameters in Parameters, set this parameter to <code>true</code>.</li>
     * <li>When you do not modify the custom parameters in Parameters, do not set this parameter or set it to <code>false</code>.</li>
     * </ul>
     * 
     * <strong>example:</strong>
     * <p>false</p>
    ...
    /**
     * <p>The modified scheduled execution frequency. This parameter takes effect only when RepeatMode is set to Period. Three types of scheduled execution are supported: fixed interval execution (based on Rate expressions), one-time execution at a specified time, and clock-based scheduled execution (based on Cron expressions).</p>
     * <ul>
     ...
     * <li>The execution interval is based only on the fixed frequency and is not related to the actual execution time of the task. For example, if the command is set to execute every 5 minutes and the task takes 2 minutes to complete, the next round of execution starts 3 minutes after the task is completed.</li>
     ...
     * </ul>
     * </li>
     * <li><p>One-time execution at a specified time: The command is executed once at the specified time zone and time point. Format: <code>at(yyyy-MM-dd HH:mm:ss &lt;time zone&gt;)</code>, which is <code>at(year-month-day hour:minute:second &lt;time zone&gt;)</code>. If no time zone is specified, UTC is used by default. The time zone supports the following three formats:</p>
     * <ul>
     * <li>Full time zone name: such as <code>Asia/Shanghai</code> (China/Shanghai time) and <code>America/Los_Angeles</code> (US/Los Angeles time).</li>
     * <li>Time zone offset from Greenwich Mean Time: such as <code>GMT+8:00</code> (UTC+8) and <code>GMT-7:00</code> (UTC-7). When using the GMT format, leading zeros are not supported for the hour value.</li>
    ...
    /**
     * <p>The instance ID of the ECS instance or managed instance to add to the scheduled task.</p>
     */
