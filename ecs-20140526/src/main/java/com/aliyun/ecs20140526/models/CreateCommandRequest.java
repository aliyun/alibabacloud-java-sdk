...
public class CreateCommandRequest extends TeaModel {
     ...
     * <ul>
     * <li><p>The value of this parameter must be Base64-encoded and cannot exceed 18 KB in size after Base64 encoding.</p>
     * </li>
     ...
     * </li>
     * <li><p><code>{{ACS::InstanceId}}</code>: the instance ID. When the command is sent to multiple instances, to specify <code>{{ACS::InstanceId}}</code> as a built-in environment parameter, make sure that Cloud Assistant Agent is not earlier than the following versions:</p>
     * <ul>
     * <li>Linux: 2.2.3.309</li>
     * <li>Windows: 2.1.3.309</li>
     * </ul>
     * </li>
     * <li><p><code>{{ACS::InstanceName}}</code>: the instance name. When the command is sent to multiple instances, to specify <code>{{ACS::InstanceName}}</code> as a built-in environment parameter, make sure that Cloud Assistant Agent is not earlier than the following versions:</p>
     * <ul>
     * <li>Linux: 2.2.3.344</li>
     * <li>Windows: 2.1.3.344</li>
     * </ul>
     * </li>
     * <li><p><code>{{ACS::InvokeId}}</code>: the command execution ID. To specify <code>{{ACS::InvokeId}}</code> as a built-in environment parameter, make sure that Cloud Assistant Agent is not earlier than the following versions:</p>
     * <ul>
     * <li>Linux: 2.2.3.309</li>
     * <li>Windows: 2.1.3.309</li>
     * </ul>
     * </li>
     * <li><p><code>{{ACS::CommandId}}</code>: the command ID. When you run a command by calling the <a href="https://help.aliyun.com/document_detail/141751.html">RunCommand</a> operation, to specify <code>{{ACS::CommandId}}</code> as a built-in environment parameter, make sure that Cloud Assistant Agent is not earlier than the following versions:</p>
     * <ul>
     * <li>Linux: 2.2.3.309</li>
     * <li>Windows: 2.1.3.309</li>
     * </ul>
     ...
     * <blockquote>
     * <p>If you specify an invalid value, the value is treated as Base64.</p>
     * </blockquote>
     * 
     * <strong>example:</strong>
     * <p>PlainText</p>
    ...
    /**
     * <p>The maximum timeout period for the command to run on the ECS instance. Unit: seconds. If the command cannot be run for some reason, a timeout occurs. After the command times out, the command process is forcefully terminated by canceling the PID of the command.</p>
    ...
    /**
     * <p>The directory where the command is run on the ECS instance. The directory cannot exceed 200 characters in length.</p>
     * <p>Default value: </p>
     * <ul>
     * <li>Linux instances: the home directory of the root user, which is the <code>/root</code> directory.  </li>
