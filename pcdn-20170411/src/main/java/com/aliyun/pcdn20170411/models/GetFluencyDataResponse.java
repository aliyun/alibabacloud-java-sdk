// This file is auto-generated, don't edit it. Thanks.
package com.aliyun.pcdn20170411.models;

import com.aliyun.tea.*;

public class GetFluencyDataResponse extends TeaModel {
    @NameInMap("headers")
    @Validation(required = true)
    public java.util.Map<String, String> headers;

    @NameInMap("body")
    @Validation(required = true)
    public GetFluencyDataResponseBody body;

    public static GetFluencyDataResponse build(java.util.Map<String, ?> map) throws Exception {
        GetFluencyDataResponse self = new GetFluencyDataResponse();
        return TeaModel.build(map, self);
    }

    public GetFluencyDataResponse setHeaders(java.util.Map<String, String> headers) {
        this.headers = headers;
        return this;
    }
    public java.util.Map<String, String> getHeaders() {
        return this.headers;
    }

    public GetFluencyDataResponse setBody(GetFluencyDataResponseBody body) {
        this.body = body;
        return this;
    }
    public GetFluencyDataResponseBody getBody() {
        return this.body;
    }

}
